...
def get_job_id(bearer_token, project_id, job_name):
    """
    Job ID 取得
    ...
    url = f"https://api.jp-tok.dataplatform.cloud.ibm.com/v2/jobs?project_id={project_id}"  # エリア指定
    ...
    return next(
        ...
        if v["metadata"]["name"] == job_name  # noqa: E501
    )


def run_job(bearer_token, job_id, project_id):
    """
    Job 実行
    ...
    url = f"https://api.jp-tok.dataplatform.cloud.ibm.com/v2/jobs/{job_id}/runs?project_id={project_id}"  # エリア指定
    headers = {"Authorization": bearer_token, "Content-Type": "application/json"}
    # Job 実行
...
def check_job_status(bearer_token, job_id, run_id, project_id):
    """
    Job ステータス確認
    ...
    url = f"https://api.jp-tok.dataplatform.cloud.ibm.com/v2/jobs/{job_id}/runs/{run_id}?project_id={project_id}"  # エリア指定
    heders = {"Authorization": bearer_token, "Content-Type": "application/json"}
    # Job ステータス取得
    response = requests.get(url, headers=heders).json()
...
def job():
    ...
    retury_times = 15  # JOB ステータス確認リトライ回数
    retry_sleep = 30  # JOB ステータス確認 sleep 期間(秒)
    for i in range(retury_times):
        job_status = check_job_status(bearer_token, job_id, run_id, project_id)
